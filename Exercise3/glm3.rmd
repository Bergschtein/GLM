...
summary(mod)
```
We have a linear model where $Y_k$ is the response variable, $X_k$ are the k-th row in the design matrix containing the covariates, $\beta$ is a vector of coefficient that are found when fitting the model and $\epsilon_k$ are the error of the model

In the summary output above, we can see that both of the covariates are significant. Expected math score increases with raven and if the gender is girl. This is consistent with what we saw in the plot obtained from using the ggpairs plot function.
...
As in **a)**, we can see that expected math score increases with `raven` and if the `gender` is girl.
...
The test $H_0$ : $\beta_{\text{raven}}=0$ against $H_1$ : $\beta_{\text{raven}}\neq0$ is performed using the anova function.
```{r}
